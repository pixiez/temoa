...
# ELASTIC: The objective now minimizes producer costs - consumer costs


def TotalWelfare_rule(M):
    r"""
The Temoa objective function maximizes the total welfare:

    Consumer cost - Producer cost

In this implementation, we minimize  (-)[Producer cost - Consumer cost].
...
The area under the Price-Quantity curve till Demand is met is the Consumer cost.
Note that area is approximated by a sum over the index z.
.. math::
   C_{consumer} & = \sum_{p, s, d, dem, z \in \Theta_{MC}} \left (
           MDemand_{p, s, d, dem, z}
     \cdot MPrice_{p, s, d, dem, z}
     \right )
...
"""
    ...
    loan_costs = sum(
        ...
        * (
            ...
            * sum((1 + GDR) ** -y
                  for y in range(S_v - P_0, S_v - P_0 + value(M.ModelLoanLife[S_t, S_v]))))
    ...
    fixed_costs = sum(
        ...
        * sum((1 + GDR) ** -y
              for y in range(S_p - P_0, S_p - P_0 + value(M.ModelTechLife[S_p, S_t, S_v]))))
    ...
    marg_costs = sum(
        ...
        * value(M.PeriodRate[S_p])) for (S_p, S_t, S_v) in M.CostMarginal.sparse_iterkeys()
    )

    producer_costs = loan_costs + fixed_costs + marg_costs
...
def ParamLoanAnnualize_rule(M, t, v):
    process = (t, v)
    annualized_rate = (
        value(M.DiscountRate[process])
        / (1 - (1 + value(M.DiscountRate[process]))
           ** (- value(M.LifetimeLoan[process]))
           )
    )
...
# ELASTIC: This constraint is not in use. Promoting demand to a variable
# makes is quadratic constraint.
...
constraint ensures that supply is equal to the V_Demand variable in all periods 
and slices.
...
.. math::
   ...
   \sum_{I, T, V} \textbf{FO}_{p, s, d, i, t, v, dem} \eq DEM_{p, s, d, dem}
...
"""
    ...
    expr = (M.V_Demand[p, s, d, dem] == M.MinDemand[p, dem] * M.DemandSpecificDistribution[s, d, dem] +
...
def DemandSegment_bounds(M, p, s, d, dem, z):
    """\
...
def DemandSegment_midpoint(M, p, s, d, dem, z):
...
def InvestmentByTechAndVintage_Constraint(M, t, v):
    ...
    investment = M.V_Capacity[t, v] * M.CostInvest[t, v]
